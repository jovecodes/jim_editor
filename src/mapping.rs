...
impl std::fmt::Debug for Mapping {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IMap").field("buttons", &self.buttons).finish()
    }
}

impl Mapping {
    pub fn new(buttons: Vec<KeyCode>, on_pressed: fn(&mut JimProperties)) -> Self { Self { buttons, on_pressed } }
        ...
        for char in self.chars() {
            if special_mode {
                ...
                if special_chars == vec!['l', 'C', 'r'] {
                    buttons.push(KeyCode::Modifier(crossterm::event::ModifierKeyCode::LeftControl));
